---
title: "Homework 7: Metapopulation dynamics"
...
***
**Due before 2:30pm Monday April 8**
...
persistence_df <- data.frame(eps = eps,
                             persistence = c(?, ?, ?, ?),
                             Patches = rep(c("2","4","8","16"), each = length(eps)))
```
...
In the next chunk, you will create a matrix to store the occupancy state (0 = unoccupied, 1 = occupied) for each site over the 25 year period. This code will also randomly generated the first year occupancy status for each patch:

```{r}
### Replace the question marks so the matrix has one row per site and one column per year
### DO NOT REPLACE THE QUESTION MARKS WITH NUMBERS!!!!
z <- matrix(NA, nrow = ?, ncol = ?)

### You also need a vector to store the proportion of sites occupied in each year
psi <- numeric(length = ?)

### This line will randomly fill in the occupancy state in year 1
z[,1] <- rbinom(n = nPatches, size = 1, prob = 0.1)

### This code will estimate the proportion of sites that are occupied in year 1
psi[1] <- mean(z[,1])
```

In years 2-25, the probability that unoccupied sites are colonized is:
...
So as long as we know the $z_{i,t}$ values, we can estimate the probability that each site is occupied using a single line of code. In the chunk below, translate the above equation into functioning `R` code:
...
for(t in 2:nYears){
  ...
  ## Calculate the proportion of sites occupied in year t
  psi[t] <- ?
}
```
