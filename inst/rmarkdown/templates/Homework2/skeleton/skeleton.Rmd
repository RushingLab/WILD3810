...
> 4b) Complete the `for` loop code to estimate `logN` in each year. Remember that `logN[t]= Count[t+1] - Count[t]`. Use that relationship to change the `?`'s on line 94 to the correct indexes. Also remember that we cannot estimate `logN` in the last year because we don't know $N_{t+1}$. Use that to change the `?` on line 93 to correct upper limit for the loop (uncomment all lines in the loop after you are done so R will run the code). 
...
# for(t in ?:?){
#  r <- ?
#  predicted_doves$Count[i] <- predicted_doves$Count[t-1] * ?
#}
```
