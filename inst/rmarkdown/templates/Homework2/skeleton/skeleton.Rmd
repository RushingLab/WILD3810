...
> 5c) What do you predict the population size of collared-doves will be in Utah in 2035? Hint - you do not need to predict any new values to answer this question. If we assume density-dependent growth, what paramter in the model determines the maximum long-term population size?   
...
> 6a) Create a new data frame called `predicted_doves` with columns `Year` and `Count`. Complete the code so that `Year` is a sequence from 2015-2035 and `Count` is `NA`  
...
> 6c) Fill in the upper and lower index values inside of `for(t in ?:?)`. Hint: Remember that we already filled in `Count[1]` so what row of the data frame should the loop start on? We want the loop to end on the final year of our predicted counts so the upper limit should correspond to the number of years in that data frame. 

> 6d) Finally, replace the `?` so that the current count (`predicted_doves$Count[t-1]`) is multiplied by the proper growth rate to predict `predicted_doves$Count[t]`. Hint - notice that the code estimates `r` in each loop based on the current population size but we are using a discrete population model based on $\lambda$.  
