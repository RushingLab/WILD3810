...
> 1c) Clicked `Knit` to check that you can create a pdf document from the .Rmd file;
...
The key to using simulations is that we **know** the true values of all of the parameters in our model. In this case, the parameters are: $N$ (the true population size) and $p$ (the probability of capturing an individual in the population). In the code chunk below (named `Np` because it is the chunk we will use to set the values of $N$ and $p$):
...
```{r nSims}
nSims <- 1000 # Number of simulated samples
```
