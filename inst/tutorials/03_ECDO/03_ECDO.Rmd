...
Packages are ways to share custom `R` functions and data that do not come with `R` itself. Packages allow anyone to contribute useful code and data for other `R` users to use as part of their analyses. The `install.packages()` function downloads the package onto your computer.
...
Notice that in the `Environment` tab in the top right panel of you RStudio window, you now see an object called `ut_dove_counts`. This object holds the data values we will use in this exercise. 
...
To see how data frames work, let's view the `ut_dove_counts` object. We can do this in several ways
...
```{r ggplot, exercise = TRUE, fig.width=8, fig.height=6, exercise.setup = "load"}
...
```{r geometry, exercise = TRUE, fig.width=8, fig.height=6, exercise.setup = "load"}
...
```{r geometry2, exercise = TRUE, fig.width=8, fig.height=6, exercise.setup = "load"}
ggplot(data = ut_dove_counts, aes(x = Year, y = Count)) + geom_line()
```

It's also possible to use more than one geometry:

```{r geometry3, exercise = TRUE, fig.width=8, fig.height=6, exercise.setup = "load"}
...
Because we want to map an *attribute* (size) to a *variable* (count), we make this change inside of `aes`:

```{r size, exercise = TRUE, fig.width=8, fig.height=6, exercise.setup = "load"}
...
```{r size2, exercise = TRUE, fig.width=8, fig.height=6, exercise.setup = "load"}
ggplot(data = ut_dove_counts, aes(x = Year, y = Count)) + geom_line() + geom_point(aes(size = Count))
```

***
The key point here - take note of this! - is that when we want to change the way the figure looks (size, color, width, etc) **based on values in the data frame**, the aesthetic **goes inside the `aes()` function**
***
...
```{r color, exercise = TRUE, fig.width=8, fig.height=6, exercise.setup = "load"}
...
```{r whitepoints, exercise = TRUE, fig.width=8, fig.height=6, exercise.setup = "load"}
...
2) Ensure that you can `Knit` the file
    ...
    2c) Save the .Rmd file as `LastnameFirstname-homework1.Rmd` in a project named `LastNameFirstName-Lab2`
...
Copy the following code to the `setup` chunk in the homework file
...
Now we can create the objects to calculate $\lambda$. 

Note that in the equation for $\lambda$, we have to divide by `N0`, which means it can't be 0 (even though we started with no doves in the state). 

So instead we'll give it a value of 1, which won't make much difference. In the box below, create an object called `N0` and give it a value of 1 
...
Copy these lines to the `calc_lambda` chunk in the homework file and run them. Use the output to answer question 
1 in the homework. 
...
In the box below, use the `filter()` function to create a new data frame called `ut_dove_counts2` containing only the non-zero counts. **What arguments do you need to provide to `filter()` to create the data frame we want?**
...
```{r lambda2, exercise = TRUE, exercise.setup = "load"}
library(dplyr)
...
<div id="lambda2-hint">
You'll need to make a new `t`, a new `N0`, and a new `NT`
...
***
Remember that instead of typing the new value of the new $\lambda$ you estimated, you can include a small bit of `R` code so that when you knit the document, the correct value is displayed. You would do this by typing `` `r "\u0060r round(lambda2, 2)\u0060"` `` within your answer (assuming the object that contains the new estimate of $\lambda$ is called `lambda2`)

***

## Predicting future population size

Your bosses at DWR want to know what the size of the collared-dove population will be over the next 20 years. Assuming they keep growing at the rate estimated in the previous step, you can use the discrete population growth model to predict annual counts of collared-doves over this time period (for now, we'll ignore the fact that BBS counts are an **index** of abundance and not an **estimate**). Remember that:

$$N_{t+1} = N_t \times \lambda$$

So if we start with the average count in 2015 (the last year we have data), we can simply use the $\lambda$ estimate to predict the count in 2016. Then we use the 2016 prediction to predict the 2017 count. And we continue until we reach the predicted 2035 count. To do this, we need to learn a new `R` skill - writing `for` loops. 
...
The `nrow()` function is your friend!
...
As mentioned earlier, we need to multiply population size in each year by $lambda$ in order to predict population size the next year. We *could* do that by typing `count[2] <- count[1] * lambda`, then `count[3] <- count[2] * lambda`, then `count[4] <- count[3] * lambda`, ..., `count[20] <- count[19] * lambda`.
