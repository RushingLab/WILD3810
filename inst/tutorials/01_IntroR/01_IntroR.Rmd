...
1) Names can be as long as you want them but rememberthat long names require more typing (so brevity is a good rule of thumb)  
...
So we see `round` takes an argument called `x`, which is the number we want to round, and the number of `digits` we want to round to. 
