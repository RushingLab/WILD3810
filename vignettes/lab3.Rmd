...
5) Name the project `PVA` and create it within your `WILD3810` folder (click `Browse` and navigate to this folder if necessary)
...
8) Save this script as `PVA_lab.R` (you will not need to turn in this script but saving it here will ensure that you can reference today's lab activities to help study for the exam)

9) Make sure you have the most update-to-date version of `WILD3810` by running:
...
- `K`: the carrying capacity (this is optional; if you do not provide `K`, the function assumes density-independent growth. For this lab, we will only focus on density-dependent growth)
...
This table returns some of the input parameters (`nYears`, `N0`, `r0`, `variance.r`) as well as summaries about the type of model (density dependent or independent), carrying capacity (if density-dependent). In addition, the output include two important summary statistics:
...
What happened to extinction probability relative to `K = 50`? To make sure the previous result wasn't a fluke, let's increase `K` again and see if the pattern holds:
...
Before moving on from the question of population size, let's look more closely at the role of demographic stochasticity by removing environmental stochasticity from the model (`variance.r = 0`) and observing how demographic stochasticity influences extinction risk at different carrying capacities. Start with a large K (`K=500`):
...
On your own, create a new data frame that contains all of the simulated results:
...
# Homework 3: Population viability of 
...
1) Ensure high probability that the population will not go extinct in the near future 

2) Ensure reasonable probability that the population does not drop below the monitoring threshold of 10
...
Because the working group contained no ecologists, the members had to out-source the hard work of actually predicting the future dynamics of the checkerspot under different management scenarios. As the agency's most promising young entomologist and an expert in population viability analysis, you have been asked to conduct a study and provide the working group with a specific recommendation for how to meet the two management objectives.  
...
2) Select `From Template`; scroll down and select `WILD3810-Homework3`

```{r out.width="100%", echo=FALSE}
knitr::include_graphics("figs/from_template.png")
```

3) In the `Name` box, type `LastnameFirstName-Homework3`
...
2) Remember that the code inserted here needs to stand alone in order for the document to knit. That means you need to load any required packages (using `library(package_name)`) **inside of .Rmd file itself**. Having the package loaded in your current working environment is not enough. 
...
6) The working group was vague about the time frame and acceptable extinction probability. You will have to decide what time frame and risk correspond to "near future" and "high probability" (or "reasonable probability" for the monitoring threshold)
...
mean(base_pva$Extinct)` ``, which would print the baseline extinction probability when you knit the document. This will save you a lot of copying/pasting, reduce typos, and ensure that if you change the code, the text will automatically be updated with the new results.   
...
With the baseline predictions, you are know ready to explore several management actions that could be taken to reduce the extinction risk of the checkerspot. These actions include:
...
You should also run the function with `extinction.threshold=10` to check the monitoring objective as well (this will create a figure called `alternative_monitoring_pva.png`).  
