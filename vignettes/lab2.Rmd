...
To see how data frames work, let's view the `ut_dove_counts` object. We can do this in several ways. The `head()` function, for example, will print the first 6 rows of the data frame:
...
```{r eval = FALSE}
ut_dove_counts2 <- filter()
```

```{r echo = FALSE}
ut_dove_counts2 <- filter(ut_dove_counts, Count > 0)
```
