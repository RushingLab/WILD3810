---
title: "Lecture 3"
subtitle: "Introduction to population growth"
...
$$N_{t+1} = N_t + \underbrace{(b \times N_t)}_{births} - \underbrace{(d \times N_t)}_{deaths}$$
--
Which can be simplied to:  

$$N_{t+1} = N_t \times (1 + b - d)$$
...
$$N_{t+1} = N_t \times \underbrace{(1 + b - d)}_{\lambda}$$
...
(p <- ggplot() + geom_path(data = Nc, aes(x = Year, y = N), linetype = "longdash", color = "#3A3F44") + geom_point(data = Nd, aes(x = Year, y = N), size = 1, color = "#7A8288"))
...
$$N_{t+2} = N_{t+1} \times \lambda$$

--
We know that $N_{t+1} = N_t \times \lambda$ so:

$$N_{t+2} = (N_t \times \lambda) \times \lambda$$
--
which simplifies to: 

$$N_{t+2} = N_t \times \lambda^2$$
---
## Discrete-time population growth model

Growth from $t$ to $t+3$:

$$N_{t+3} = N_t \times \lambda^3$$

So we get the general form $^2$:

$$N_{T} = N_0 \times \lambda^T$$
...
Example of birth-pulse species include most birds, 
...
q <- p + geom_segment(aes(x = 5, xend = 5, y = -Inf, yend = Nd$N[5]), 
  ...
  geom_segment(aes(x = -Inf, xend = 10, y = Nd$N[10], yend = Nd$N[10]), 
               color = WILD3810_colors$value[WILD3810_colors$name == "secondary"]) +
  geom_segment(aes(x = 5, xend = 10, y = Nd$N[5], yend = Nd$N[10]), 
...
q <- p + geom_segment(aes(x = 5, xend = 5, y = -Inf, yend = Nd$N[5]), 
  ...
  geom_segment(aes(x = -Inf, xend = 6, y = Nd$N[6], yend = Nd$N[6]), 
               color = WILD3810_colors$value[WILD3810_colors$name == "secondary"]) +
  geom_segment(aes(x = 5, xend = 6, y = Nd$N[5], yend = Nd$N[6]), 
               color = WILD3810_colors$value[WILD3810_colors$name == "danger"])
q
```

$\Delta_t = 1$ and $\Delta_N$ = `r round(Nd$N[6] - Nd$N[5],2)` $^4$
...
q <- p + geom_segment(aes(x = 5, xend = 5, y = -Inf, yend = Nd$N[5]), 
  ...
  geom_segment(aes(x = 5, xend = 5.25, y = Nd$N[5], yend = Nc$N[89]), 
               color = WILD3810_colors$value[WILD3810_colors$name == "danger"]) +
  geom_abline(slope = 1.305, intercept = 8.072, 
              color = WILD3810_colors$value[WILD3810_colors$name == "danger"])
q
```
...
$$\frac{dN}{dt} = N \times (b-d) = N \times r$$
...
How does $r$ compare to $\lambda$? 

$$r = log(\lambda)$$
$$\lambda = e^r$$
So for a continuous-time model, we can project population growth by substituting $e^r$ in the equation we used to project discrte population growth:
...
In other words, what is $T$ when $N_T = 2 \times N_0$? 
...
To isolate $rT$, take the natural log $^6$ of both sides:

$$ln(2) = ln({e^r}^T) = rT$$

Now divide both sides by $r$:

$$\frac{0.693}{r} = T_{double}$$
...
r <- ggplot() + geom_path(data = Ndouble, aes(x = t, y = N, group = r, color = r)) +
  scale_color_manual(values = WILD3810_colors$value[WILD3810_colors$name %in% c("primary", "danger", "warning", "success")]) +
  geom_segment(data = segment_df, aes(x = x, xend = xend, y = y, yend = yend, color = r)) +
  geom_text(data = dt_df, aes(label = paste("Doubling time = ", dt, sep = ""), x = 20, y = 5500), color = "#7a8288") + 
  facet_wrap(~r, nrow = 2) +
  theme(legend.position = "none", strip.text.x = element_text(hjust = 0.025))
...
--
+ After 10 years, the investment will be worth $`r round(1000 * 1.067^10,2)` (~100% return)  
<br/>
...
$$\lambda = \bigg(\frac{N_{T}}{N_0}\bigg)^{\frac{1}{T}}$$

--
What was the growth rate of the sheep and pheasant populations?  

--
+ Sheep: $\bigg(\frac{2000000}{200000}\bigg)^{\frac{1}{30}}=$ `r round((2000000/200000)^(1/30),2)`  

--
+ Pheasants: $\bigg(\frac{1325}{6}\bigg)^{\frac{1}{6}}=$ `r round((1325/6)^(1/5), 2)`
