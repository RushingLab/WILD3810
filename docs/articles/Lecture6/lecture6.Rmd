...
Remember from the previous lecture that our simple models of population growth were based on the following assumptions:  
<br/>
...
--
The B-D population model ignored all of these sources of variation $^1$

???

$^1$ As we'll see, this is not only unrealistic, ignoring variation can change our conclusions about population growth. 
...
--
Just because something is stochastic doesn't mean it's completely unpredictable $^1$  
...
(dem_plot <- ggplot(Nd, aes(x = t, y = N, group = Population)) + geom_path(color = WILD3810_colors$value[WILD3810_colors$name == "warning"], alpha = 0.25))
```

???

$^1$ For example, we can't predict with 100% certainty whether a coin flip will be heads or tails or what side a 6-sided dice will land on (there is some stochasticity in these processes) but that doesn't mean we have no idea what the outcomes will be  
...
--
Probability distributions characterize the frequency of different possible outcomes $^1$  
...
$^1$ For example, many of you are probably familiar with the normal (or bell-shaped) distribution. This is a probability distribution with a single "most probable" outcome and decreasing probability of more "extreme" values.
...
r + geom_rug(data = years_df, sides = "b", color = WILD3810_colors$value[WILD3810_colors$name == "info"]) +
...
$$N_{t+1} = N_t \times (1 + b_t - d_t)$$

--
$$N_{t+1} = N_t \times \lambda_t$$

--
Over $T$ years, abundance will be:

$$N_{T} = N_0 \times \big(\lambda_0 \times \lambda_1 \times \lambda_2 \times...\lambda_{T-1}\big)$$
...
$$N_{5} = 50 \times 1.1 \times 0.9 \times 0.7 \times 1.2 \times 1.1 = 46$$  

```{r fig.height=2.5, fig.width=4}

lambda <- prod(c(1.1,0.9,0.7,1.2,1.1))^(1/5)

Ns <- data.frame(Time = 1:5,
                 N.pred = 50,
                 N = c(55, 49.5, 34.65, 41.48, 46))
...
--
What *is* the average $\lambda$ (which we'll call $\bar{\lambda}$?  

---
## Modeling environmental stochasticity

An obvious way to calculate $\bar{\lambda}$ is the *arithmetic mean* of the annual $\lambda$'s:

$$\bar{\lambda} = \frac{\lambda_0+ \lambda_1 + \lambda_2 +...\lambda_{T-1}}{T}$$

--
In our example, this equals:

$$\bar{\lambda} = \frac{1.1 + 0.9 + 0.7 + 1.2 + 1.1}{5} = \frac{5}{5} = 1$$
---
## Modeling environmental stochasticity
...
$\bar{\lambda} = 1$ means the population should have, on average, remained at 50 individuals 
<br/>
<br/>
...
This means that shrinking by 30% $(\lambda = 0.7)$ and then growing by 30% $(\lambda = 1.3)$ does not get you back to where you started  
...
$$\bar{\lambda} = \big(\lambda_0 \times \lambda_1 \times \lambda_2 \times...\lambda_{T-1}\big)^\frac{1}{T}$$  


--
For our population, that means: 

$$\bar{\lambda} = \big(1.1 \times 0.9 \times 0.7 \times 1.2 \times 1.1\big)^\frac{1}{5} = 0.98$$
...
--
Initially, assume a large population ($N_0 = 500$) with:
...
$$\bar{\lambda} = (1 + \bar{b_t} - \bar{d_t}) = (1 + 0.55 - 0.5) = 1.05$$
...
ggplot(data = Ns, aes(x = Time, y = N)) + geom_path(color = WILD3810_colors$value[WILD3810_colors$name=="secondary"]) 
  # scale_color_manual(values = c(WILD3810_colors$value[WILD3810_colors$name=="secondary"]))
```
]
...
ggplot(data = Ns, aes(x = Time, y = N, color = Population)) + geom_path() +
  scale_color_manual(values = c(WILD3810_colors$value[WILD3810_colors$name=="warning"], WILD3810_colors$value[WILD3810_colors$name=="secondary"])) +
...
Because environmental stochasticity tends to reduce $\bar{\lambda}$ regardless of population size, it has important consequences for the extinction risk of **both small and large populations**  
...
Likewise, we might expect individuals to produce 3 offspring on average, but some individuals will have more and some fewer based on chance $^1$  
...
???
$^1$ or at least what appears to use to be chance  
...
--
Start with 100 populations with a relatively large population size $(N = 500)$ $^1$  

We will further assume that the mean survival and reproductive rates remain constant so all stochastisticity is demographic  
...
$^1$ Since each population starts with the same initial populaiton size, any differences at the end of the simulation are due to stochasticity  

---
## Effects of demographic stochasticity

In this example, `r mean(Nd$N[Nd$t==30] < 500)*100`% of the populations decreased $(N_T < 500)$ and none went extinct   
...
Now `r mean(Nd$N[Nd$t==30] < 10)*100`% of the populations decreased and `r mean(apply(N, 1, min)==0)*100` went extinct $^1$ 
...
$^1$ Remember that if at any point during the timeseries $N=0$, the population is extinct. Because our model assumes no movement, a population that goes extinct stays extinct
...
```{r  fig.width=8, fig.height=5}
ggplot(Nd[Nd$t < 51, ], aes(x = t, y = N, group = Population)) + 
...
```{r fig.width=8, fig.height=5}
ggplot(Nd[Nd$t < 201, ], aes(x = t, y = N, group = Population)) + 
  geom_path(color = WILD3810_colors$value[WILD3810_colors$name == "warning"], alpha = 0.25)# +
  # transition_reveal(id = Population, along = t) +
  # enter_appear() +
  # exit_disappear() +
...
```{r fig.width=8, fig.height=5}
ggplot(Nd[Nd$t < 501, ], aes(x = t, y = N, group = Population)) + 
  geom_path(color = WILD3810_colors$value[WILD3810_colors$name == "warning"], alpha = 0.25) #+
  # transition_reveal(id = Population, along = t) +
  # enter_appear() +
  # exit_disappear() +
...
10,000 years? `r mean(apply(N, 1, min)==0)*100`% of populations go extinct $^1$ 
...
$^1$ We could go further if we wanted and that last population would eventually go extinct as well  
